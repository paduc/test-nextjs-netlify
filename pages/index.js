...
import { attributes, react as HomeContent } from '../content/landing.md';
...
export default class Home extends Component {
  render() {
    let { ministère, nomService, sousTitreService, liensEntête, menu, liensFooter, descriptionFooter } = attributes;
        ...
        <div className="fr-header__tools-links">
            <ul className="fr-links-group">
              { liensEntête.map(({ titre, url }, index) => (
                ...
                <a className="fr-link fr-fi-external-link-line fr-link--icon-right" href={url} title={titre} target="_blank" rel="noopener">{titre}</a>
              </li>
              ))}
            </ul>
          </div>
      ...
      <nav className="fr-nav" role="navigation" aria-label="Menu principal" id="header-navigation">
        <ul className="fr-nav__list">
          ...
          (<li key={`menu${index}`} className="fr-nav__item">
            <a className="fr-nav__link" href={url} target="_self">{titre}</a>
          </li>))}
        </ul>
      </nav>
    </div>
            ...
            <p className="fr-logo">
              {Nl2Br(ministère)}
                ...
                <div className="fr-footer__content">
                  ...
                  <ul className="fr-footer__content-list">
                    ...
                    liensFooter.map(({titre, url}, index) => (<li className="fr-footer__content-item" key={`footer_${index}`}>
              ...
              <div className="fr-footer__bottom">
                <ul className="fr-footer__bottom-list">
                  ...
                  <li className="fr-footer__bottom-item">
                    <a className="fr-footer__bottom-link" href="/contact">Contactez-nous</a>
                  </li>
                  <li className="fr-footer__bottom-item">
                    <button className="fr-footer__bottom-link fr-fi-sun-fill-line fr-link--icon-left" aria-controls="fr-theme-modal" data-fr-opened="false" data-fr-js-modal-button="true">Paramètres d'affichage</button>
                  </li>
                </ul>
